...
GAME_ID = "769275b6-3fcf-4ce5-8aa8-d18d24064619"
...
def paste_player_data(players):
    ...
    if not BOMB_PLAYER_MUL_ENABLE:
        if EF_PLAYER["dragonEggDelay"] >= 3:
            BOMB_PLAYER_MUL_ENABLE = True
...
def ticktack_handler(data):
    global BOMB_POS_LOCK_BASE, COUNT, COUNT_UPDATE, COUNT_ST, ACTION_PER_POINT
    global TIME_POINT
    ...
    if COUNT == ACTION_PER_POINT:
        print("trigger case 1")
    elif data["timestamp"] - TIME_POINT > RANGE_TIME:
        print("trigger case 2")

    if COUNT == ACTION_PER_POINT or data["timestamp"] - TIME_POINT > RANGE_TIME:
        ACTION_PER_POINT = 2
        TIME_POINT = data["timestamp"]
        BOMB_POS_LOCK_BASE = get_list_pos_bomb_danger(data["map_info"]["bombs"])  # update liên tục
        ...
        if len(direction) < ACTION_PER_POINT:
...
def a_star(start, target):
    ...
    while queue:
        ...
        for act in NextMoveZone.Z4.value:
            ...
            if new_pos_player in BOMB_POS_LOCK_BASE:
                continue
...
def next_pos_bfs(actions, cr_status, pos_list, size_map, queue: list):
    for act in actions:
        ...
        if point >= 25 and new_pos_player not in BOMB_POS_LOCK_BASE:  # not is_danger_bombs(new_pos_player, BOMBS):
    ...
    if position.bombs:
        for bomb in position.bombs:
            ...
            if list_pos is not None:
                for idx, x in enumerate(list_pos):
                    if is_danger_bomb(x, bomb):
                        value += match_idx_bomb(idx)
    else:
        pass
    if list_pos is not None:
        for idx, x in enumerate(list_pos[0:4]):
            if x in SPOILS_POS:
...
# todo : set global var de ngat direction tai point no danger
...
def minimax_no_e(position: Position, zone: int) -> list:
    ...
    try:
        ...
        pass
    print("value:", value, " pos_list: ", pos_list)
    return act_list
...
def move_action(position: Position, current_action, actions, level, pos_list, move_list):
    ...
    if new_pos_player in BOMB_POS_LOCK_BASE:  # and BOMB_PLAYER_ENABLE
        return MIN, pos_list, move_list
    """
